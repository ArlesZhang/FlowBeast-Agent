...
def main():
    parser = argparse.ArgumentParser(description="FlowBeast FP3 自动回流工具")
    ...
    if args.file:
        feedback.process_file(Path(args.file), auto_confirm=args.yes)
    elif args.dir:
        target_dir = Path(args.dir)
        for json_file in target_dir.glob("*.json"):
            feedback.process_file(json_file, auto_confirm=args.yes)
    else:
        # 默认处理输出目录
